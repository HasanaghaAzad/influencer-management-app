      ...
      <div className="bg-gray-800">
            ...
            <div className="hidden md:block">
              <div className="ml-4 flex items-center md:ml-6">
                <Menu className="relative ml-3">
                  ...
                  <DropdownButton title={user.name} items={userNavigation} />
                </Menu>
              </div>
            </div>
        ...
        <div className="md:hidden">
          <div className="space-y-1 px-2 pb-3 pt-2 sm:px-3">
            {navigation.map((item) => (
              <a key={item.label} href={item.href} aria-current={item.current ? "page" : undefined} className={classNames(item.current ? "bg-gray-900 text-white" : "text-gray-300 hover:bg-gray-700 hover:text-white", "block rounded-md px-3 py-2 text-base font-medium")}>
                {item.label}
              </a>
            ))}
          </div>
          <div className="border-t border-gray-700 pb-3 pt-4">
            ...
            <div className="mt-3 space-y-1 px-2">
              {userNavigation.map((item) => (
                <a key={item.label} href={item.href} className="block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white">
                  {item.label}
                </a>
              ))}
            </div>
